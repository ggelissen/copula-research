# R Code for Definitive Convergence Analysis (n=10 to 1000)
#
...
# 2. Difficult: t Copula + t Margins
#
# It uses a high N_sim=500 for smooth, low-noise results.
...
# WARNING: N_sim=500 will result in a VERY long runtime (hours).
# For a quick test, set N_sim=10. For the real run, use >= 500.
...
param_grid <- rbind(grid_normal, grid_t)
param_grid$family <- factor(param_grid$family)
param_grid$margins <- factor(param_grid$margins)

cat("Starting DEFINITIVE n-convergence simulation.\n")
cat("N_sim =", N_sim, "\n")
cat("Scenarios to run:", nrow(param_grid), "\n")
cat("This will take several hours to complete.\n")
...
run_scenario <- function(N_sim, n_obs, family, margins, true_tau) {
  ...
  # Check for empty vectors (if all fits failed, very unlikely)
...
# 6.1. Create 'MSE_Inflation' and 'Scenario' metrics
...
# This plot will have 4 lines:
# (Normal/norm, known), (Normal/norm, unknown)
# (t/t, known), (t/t, unknown)
gg_mse_decay <- ggplot(results_long_mse, aes(x = n_obs, y = MSE, color = Scenario, linetype = Case)) +
  ...
  labs(
    ...
    subtitle = "Comparing 'Normal/norm' (Baseline) vs. 't/t' (Difficult)",
    x = "Sample Size (n)",
...
# This plot will have 2 lines:
# (Normal/norm), (t/t)
gg_cost_decay <- ggplot(results_df, aes(x = n_obs, y = MSE_Inflation, color = Scenario)) +
  geom_line(size = 1, alpha = 0.8) +
  geom_point(alpha = 0.2, size = 1) + # Show the (now less noisy) points
  ...
  labs(
    title = "Relative Cost of Unknown Margins vs. SampleSize",
    subtitle = "Comparing 'Normal/norm' (Baseline) vs. 't/t' (Difficult)",
    x = "Sample Size (n)",
